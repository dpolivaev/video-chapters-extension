  ...
  class YouTubeIntegration {
    ...
    async getSubtitleData(sendResponse) {
      ...
      if (this.isProcessing) {
        ...
        sendResponse({
          success: false,
          error: "Already processing subtitle data."
        });
        return;
      }
      ...
      if (!videoId) {
        ...
        sendResponse({
          success: false,
          error: "No video ID found. Please make sure you are on a YouTube video page."
        });
        return;
      }
      try {
        ...
        const subtitleData = {
          ...
          extractor: this.subtitleExtractor
        };
    ...
    async getTranscriptDict(videoUrl) {
      ...
      const {title: title, author: author, ytData: ytData, dataKey: dataKey, resolvedType: resolvedType} = await this.resolveYouTubeData(videoUrl, dataType);
    ...
    async resolveYouTubeData(videoUrl, initialType) {
      ...
      let title = ytData?.videoDetails?.title || ytData?.contents?.twoColumnWatchNextResults?.results?.results?.contents?.[0]?.videoPrimaryInfoRenderer?.title?.runs?.[0]?.text || ytData?.playerOverlays?.playerOverlayRenderer?.videoDetails?.playerOverlayVideoDetailsRenderer?.title?.simpleText || document.querySelector("h1.ytd-watch-metadata yt-formatted-string")?.textContent || document.querySelector('meta[property="og:title"]')?.content || document.title.replace(" - YouTube", "") || "Untitled";
      let author = ytData?.videoDetails?.author || ytData?.contents?.twoColumnWatchNextResults?.results?.results?.contents?.[1]?.videoSecondaryInfoRenderer?.owner?.videoOwnerRenderer?.title?.runs?.[0]?.text || document.querySelector("#owner #channel-name a")?.textContent || document.querySelector("ytd-channel-name a")?.textContent || document.querySelector('meta[name="author"]')?.content || "Unknown Channel";
      ...
      const hasTranscriptPanel = panels.some(p => p.engagementPanelSectionListRenderer?.content?.continuationItemRenderer?.continuationEndpoint?.getTranscriptEndpoint);
    ...
    async getTranscriptItems(ytData, dataKey) {
      ...
      const continuationParams = ytData.engagementPanels?.find(p => p.engagementPanelSectionListRenderer?.content?.continuationItemRenderer?.continuationEndpoint?.getTranscriptEndpoint)?.engagementPanelSectionListRenderer?.content?.continuationItemRenderer?.continuationEndpoint?.getTranscriptEndpoint?.params;
    ...
    createTranscriptArray(items, type) {
      return type === "regular" ? items.map(item => this.getSegmentData(item)) : items.filter(e => e.segs).map(e => this.getShortsSegmentData(e));
    }
    getSegmentData(item) {
    ...
    extractJsonFromHtml(html, key) {
      const regexes = [ new RegExp(`window\\["${key}"\\]\\s*=\\s*({[\\s\\S]+?})\\s*;`), new RegExp(`var ${key}\\s*=\\s*({[\\s\\S]+?})\\s*;`), new RegExp(`${key}\\s*=\\s*({[\\s\\S]+?})\\s*;`) ];
