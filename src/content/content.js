...
function extractTimestampFromSegment(segment) {
  if (!segment || typeof segment.querySelector !== 'function') {
    return '';
  }

  return segment.querySelector('.segment-time')?.textContent?.trim() ||
    segment.querySelector('#segment-time')?.textContent?.trim() ||
    segment.querySelector('.segment-timestamp')?.textContent?.trim() ||
    '';
}

function buildTranscriptEntriesFromSegments(segments) {
  const entries = [];
  let missingTimestampCount = 0;

  segments.forEach(segment => {
    const timestamp = extractTimestampFromSegment(segment);
    const textElement = segment.querySelector('.segment-text') || segment.querySelector('#segment-text');
  ...
  if (!entries.length || missingTimestampCount > entries.length / 2) {
...
function formatTranscriptLines(entries) {
  return entries.map(entry => entry.timestamp ? `(${entry.timestamp}) ${entry.text}` : entry.text);
}
    ...
    class YouTubeIntegration {
      ...
      async extractTranscriptData() {
        try {
          ...
          const lines = formatTranscriptLines(transcriptObj.transcript).join('\n');
      ...
      async scrapeTranscriptFromDom() {
        ...
        try {
          transcript = buildTranscriptEntriesFromSegments(segments);
        } finally {
          if (openedByScript) {
            this.closeTranscriptPanel(panel);
          }
